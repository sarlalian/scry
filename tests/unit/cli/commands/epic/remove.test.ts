...
describe("epic remove command", () => {
  test("removes single issue from epic", async () => {
    const mockUpdate = mock(
      async (issueKey: string, fields: Record<string, unknown>): Promise<void> => {
        expect(issueKey).toBe("PROJ-123");
        expect(fields).toEqual({ parent: null });
      }
    );

    await mockUpdate("PROJ-123", { parent: null });

    expect(mockUpdate).toHaveBeenCalledTimes(1);
    expect(mockUpdate).toHaveBeenCalledWith("PROJ-123", { parent: null });
  });

  test("removes multiple issues from epic", async () => {
    const mockUpdate = mock(
    ...
    await mockUpdate("PROJ-123", { parent: null });
    await mockUpdate("PROJ-124", { parent: null });
    await mockUpdate("PROJ-125", { parent: null });
  ...
  test("formats success message for single issue", () => {
    const formatMessage = (
      ...
      if (issueKeys.length === 1) {
        return {
          success,
          message: `Issue ${issueKeys[0]} has been removed from epic ${epicKey}`,
        };
      }
      return {
        success,
        message: `${issueKeys.length} issues have been removed from epic ${epicKey}`,
      };
    };

    const result = formatMessage("PROJ-100", ["PROJ-123"], true);
    expect(result.success).toBe(true);
    expect(result.message).toContain("Issue PROJ-123 has been removed from epic PROJ-100");
  });
  ...
  test("formats success message for multiple issues", () => {
    ...
    expect(result.success).toBe(true);
    expect(result.message).toContain("3 issues have been removed from epic PROJ-100");
  });
  ...
  test("updates parent field to null", () => {
  ...
  test("verifies issue was removed from the correct epic", async () => {
    const mockGet = mock(async (issueKey: string) => {
      return {
        key: issueKey,
        fields: {
          parent: { key: "PROJ-100" },
    ...
    expect(issue.fields.parent.key).toBe("PROJ-100");
  });
});
